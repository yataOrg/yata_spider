...
class YataspiderItem(scrapy.Item):
    ...
    def get_insert_sql(self):
        ...
        insert_sql = """
                   ...
                   opening_hours2, opening_hours3, phone1, phone2, rating, order_lead_time, recommend_reasons, created_at, updated_at)
                   VALUES ('%s', %f, %f, '%s', %f, %f, '%s', '%s', '%s', '%s', '%s', '%s', %f, '%s', '%s', '%s', '%s');
               """
        params = (
        self["address"], self["float_delivery_fee"], self["float_minimum_order_amount"], self["cuisine"], self["latitude"], self["longitude"],
            ...
            self["order_lead_time"], self["recommend_reasons"], now_time, now_time
        )
        print("insert data ***" * 6)
